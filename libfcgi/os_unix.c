...
#include <sys/socket.h> /* for getpeername/getsockname */
#endif
 ...
 * OS_OpenStream --
 *
 *   This procedure is responsible for checking the file stream
 *   on Unix for local process communication.
 *
 * Results:
 *      This call returns either a valid file descriptor or -1 on error.
 *
 * Side effects:
 *      None.
 *
 *----------------------------------------------------------------------
 */
int OS_OpenStream(FILE *stream)
{
    return stream && !ferror(stream) ? fileno(stream) : -1;
}
...
/* Returns TRUE if sockfd is a socket file descriptor, or FALSE otherwise. */
static int is_socket(const int sockfd)
{
	union {
    ...
    errno = 0;
    const int r = getsockname(sockfd, (struct sockaddr *)&sa, &len);
    const int e = errno;
    errno = 0;
    return (r == 0 || (e != EBADF && e != ENOTSOCK)) ? TRUE : FALSE;
}
 ...
 * OS_IsFile --
 *
 *	Determines whether the file descriptor sockfd refer to a file or not.
 *
 * Results:
 ...
 *----------------------------------------------------------------------
 */
int OS_IsFile(const int sockfd)
{
    return !is_socket(sockfd);
}
