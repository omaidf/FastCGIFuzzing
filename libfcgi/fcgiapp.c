...
int FCGX_GetChar(FCGX_Stream *stream)
{
    ...
    ASSERT(OS_IsFile(((FCGX_Stream_Data *)stream->data)->reqDataPtr->ipcFd) || stream->isClosed); /* bug in fillBufProc if not */
    return EOF;
}
    ...
    for (;;) {
        ...
        switch(status) {
            case STREAM_RECORD:
                ...
                if(data->contentLen == 0) {
                    stream->wrNext = stream->stop = stream->rdNext;
                    if (!OS_IsFile(data->reqDataPtr->ipcFd)) {
                        stream->isClosed = TRUE;
                    }
                    return;
	        }
...
int FCGX_OpenStream(FILE *stream)
{
    return OS_OpenStream(stream);
}
...
int FCGX_Accept_r(FCGX_Request *reqDataPtr)
{
    ...
    const int is_file = OS_IsFile(reqDataPtr->ipcFd);
