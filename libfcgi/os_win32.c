...
static int Win32NewDescriptor(FILE_TYPE type, int fd, int desiredFd)
{
    ...
    if (index != -1) 
    {
        if (type == FD_FILE_SYNC)
            fdTable[index].fid.fileHandle = (HANDLE) _get_osfhandle(fd);
        else
 ...
 * OS_OpenStream --
 *
 *   This procedure is responsible for associating a pseudo-descriptor
 *   with the given stream.
 *
 * Results:
 *      This call returns either a valid pseudo file descriptor or -1 on error.
 *
 *----------------------------------------------------------------------
 */
int OS_OpenStream(FILE *stream)
{
    ...
    const int pseudoFd = Win32NewDescriptor(FD_FILE_SYNC, fd, fd);
    if (pseudoFd == -1) {
        fclose(stream);
        return -13;
    }

    listenType = FD_FILE_SYNC;
...
static int fileFd(const int listen_sock)
{
    int result = -1;
    EnterCriticalSection(&fdTableCritical);
    if (listen_sock < WIN32_OPEN_MAX && fdTable[listen_sock].type == FD_FILE_SYNC)
        result = listen_sock;
    LeaveCriticalSection(&fdTableCritical);
    return result;
}
...
int OS_Accept(int listen_sock, int fail_on_intr, const char *webServerAddrs)
{
    ...
    else if (listenType == FD_FILE_SYNC)
    {
        ipcFd = fileFd(listen_sock);
    }
